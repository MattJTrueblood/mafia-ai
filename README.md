...
- Web application front end where the user selects any number of players (default 7) and their models.  Each player is an LLM model.  Roles are distributed secretly to each model.
- The game starts on the night phase so that the game starts with one death.
- The game is divided between day and night phases.
    -Day phase:
        ...
        -Voting phase:  The surviving players will vote on who to lynch.  They may output a short explanation of their vote, and then must publicly vote using structured outputs only.  They may vote on a player or abstain.  Votes and explanations are done sequentially, so that e.g. an unexpected earlier vote might change who a later player decides to vote for.
    -Night phase:
        -The mafia gather to vote on who to kill next.  The vote is done similarly to the voting in the day phase, using structured outputs only.
        -The doctor decides who to protect.  They cannot protect the same person twice in a row.  They can choose to protect themselves or abstain as well.  That person cannot be killed if they are targeted that night (by the mafia OR the vigilante).  Using structured outputs only.  
        -The sheriff decides who to investigate.  Whether that player is mafia or town is revealed to them.  Using structured outputs only.
